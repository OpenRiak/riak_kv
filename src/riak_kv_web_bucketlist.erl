...
match_route('GET', _, [<<"types">>, T, <<"buckets">>]) ->
    {
        ok,
        {32, 2048, 0},
        #context{bucket_type = T}
    };
match_route(Method, _, [<<"types">>, _T, <<"buckets">>]) when
    Method =/= 'GET'
->
    {method_not_allowed, ['GET']};
match_route(Method, Path, [<<"buckets">>]) ->
    match_route(
        Method,
        Path,
        [<<"types">>, <<"default">>, <<"buckets">>]
    );
match_route(_, _, _) ->
    nomatch.
...
parse_query_params(Params, Ctx) ->
    Ctx1 =
        case lists:keyfind(<<"buckets">>, 1, Params) of
            {<<"buckets">>, <<"stream">>} ->
...
process_request(none, #context{bucket_type = BT, client = C} = Ctx) ->
    case Ctx#context.stream of
        true ->
            {ok, ReqId} =
                riak_client:stream_list_buckets(
                    none,
                    Ctx#context.timeout,
                    BT,
                    C
...

            {
                ok,
                {
                    200,
                    [?JSN_HEADER],
                    {stream, bucket_stream_fun(ReqId)},
                    true,
                    none
                },
                Ctx
        ...
        false ->
            case riak_client:list_buckets(none, Ctx#context.timeout, BT, C) of
                {ok, Buckets} ->
                    JsonResults =
                        riak_kv_wm_json:encode(
                            #{<<"buckets">> => handle_unicode_buckets(Buckets)}
                        ),
                    {
                        ok,
                        {
...
-spec bucket_stream_fun(non_neg_integer()) -> stream_fun().
bucket_stream_fun(ReqId) ->
    fun() ->
        receive
...
-spec handle_unicode_buckets(list(binary())) -> list(binary()).
handle_unicode_buckets(Buckets) ->
    % lists:map(fun uri_string:quote/1, Buckets).
    Buckets.
...
parse_test() ->
    InitCtx = #context{bucket_type = <<"T">>},
    {ok, Ctx1} =
        parse_query_params(
            [{<<"buckets">>, <<"stream">>}, {<<"timeout">>, <<"1000">>}],
            InitCtx
        ),
