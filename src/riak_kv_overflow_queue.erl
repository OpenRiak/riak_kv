...
log(Type, JobID, Attempts, Aborts, Queue) ->
    case has_non_zero_log_data(Attempts, Aborts, sum_queue_stats(Queue)) of
        true ->
            QueueLengths =
...
%% @private
has_non_zero_log_data(0, 0, 0) ->
    false;
has_non_zero_log_data(_, _, _) ->
    true.

%% @private
sum_queue_stats(Queue) ->
    QueueStatLengthsList = [QueueStatLengths || {_, QueueStatLengths} <- stats(Queue)],
    lists:sum([
        lists:sum([L || {_, L} <- QueueStatLengths])
           || QueueStatLengths <- QueueStatLengthsList
    ]).
